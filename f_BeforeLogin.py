  ...
  if (len(accounts) >= MAX_ACCOUNTS):
    ...
    while (1):
      ...
      for i in accFullName.keys():
        if (
            i == fullname
        ):  #Checks to see if the first name and last name are a unique combination
    ...
    while (1):
      ...
      if (hasCapital and hasDigit and hasSpecial):
        ...
        accFullName[
            fullname] = username  #Saves the firstName and lastName as the key and the username as the value
...
def connectPeople():
  ...
  firstNameSearch = input(
      str("Please enter the first name of the person you are looking for: "))
  ...
  if fullNameSearch in accFullName:  #Searchs through names of registered users
    ...
    searchJoin = int(
        input("Please select 1 to log in or select 2 to sign up: "))
